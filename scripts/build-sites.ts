 ...
 */
import { buildSites, splitSites, type BlmSite, type CspSite, type OsmSite, type RidbExtra, type RidbSite, type SitePage } from '../src/lib/mergeSites'
...
const sites = buildSites(fc, await read<Record<string, SitePage>>('site-pages.json', {}), await read<RidbSite[]>('ridb-sites.json', []), await read<Record<string, RidbExtra>>('ridb-extra.json', {}), await read<CspSite[]>('csp-sites.json', []), await read<OsmSite[]>('osm-sites.json', []), await read<BlmSite[]>('blm-sites.json', []))
