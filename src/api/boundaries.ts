...
import { buildSites, type BlmSite, type CspSite, type RidbUnlocated, type OsmSite, type RecSite, type RidbExtra, type RidbSite, type SitePage } from '../lib/mergeSites'
...
export function useRecSites() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const [fc, pages, ridb, extra, csp, osm, blm, unlocated] = await Promise.all([snapshotOrLive<GeoJSON.FeatureCollection<GeoJSON.Point>>('sites'), local<Record<string, SitePage>>('site-pages.json', {}), local<RidbSite[]>('ridb-sites.json', []), local<Record<string, RidbExtra>>('ridb-extra.json', {}), local<CspSite[]>('csp-sites.json', []), local<OsmSite[]>('osm-sites.json', []), local<BlmSite[]>('blm-sites.json', []), local<RidbUnlocated[]>('ridb-unlocated.json', [])])
      return buildSites(fc, pages, ridb, extra, csp, osm, blm, unlocated)
    },
  })
}
