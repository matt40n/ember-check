  ...
  return useQuery({
    ...
    queryFn: () =>
      fetchJson<GeoJSON.FeatureCollection>(
        `${BASE}/WFIGS_Interagency_Perimeters_Current/FeatureServer/0/query?where=1%3D1&${bbox}&outFields=poly_IncidentName,poly_GISAcres,attr_PercentContained&outSR=4326&f=geojson`,
      ),
  })
}

/** NFDRS 3.0 predictive service areas for the Northern California GACC — ERC percentile is what GACCs use to bin fire danger. */
export function useFireDanger() {
  return useQuery({
    queryKey: ['ca-nfdrs-psa'],
    refetchInterval: 24 * 60 * 60_000, staleTime: 24 * 60 * 60_000,
    queryFn: () =>
      fetchJson<GeoJSON.FeatureCollection>(
        `${BASE}/CA_NFDRS/FeatureServer/1/query?where=GACCUnitID%3D%27ONCC_CA%27&outFields=PSAName,PSANationalCode,Avg_ERC,Avg_ERC_Pct,Avg_BI,Avg_ERC_Trend,EditDate&outSR=4326&f=geojson`,
      ),
  })
}
