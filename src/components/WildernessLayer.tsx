  ...
  return (
    <GeoJSON
      ...
      onEachFeature={(f, l) => {
        ...
        l.bindTooltip(
          by
            ? `<b>${esc(name)}</b><br/>Campfires allowed with a CA Campfire Permit<br/><small>exemption in ${esc(by.map((j) => `${j.name} order${j.orderNumber ? ` ${j.orderNumber}` : ''}`).join('; '))}${by[0].wildernessNote ? `<br/>${esc(by[0].wildernessNote)}` : ''}</small>`
            : `<b>${esc(name)}</b><br/>No campfire exemption — enclosing order applies`,
